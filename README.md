...
- **Формат пакета:** `[Header: 28 байт][Payload: N байт][CRC32: 4 байта]`
...
| **Заголовок** | 28 байт | Пакетный заголовок (`PacketHeader`) |
| **Payload** | N байт | Полезная нагрузка в формате JSON |
| **CRC32** | 4 байта | Контрольная сумма для заголовка и payload |
...
│  0x08          4               PAYLOAD_LENGTH         Длина полезной нагрузки    │
│  0x0C          8               TIMESTAMP              Временная метка (unix ms)  │
│  0x14          4               SEQUENCE_NUMBER        Номер последовательности   │
│  0x18          4               RESERVED               Зарезервировано             │
│                                                                                  │
└──────────────────────────────────────────────────────────────────────────────────┘
```
...
│  │                                                        │   │
│  │ decode():                                              │   │
│  │   1. Проверка PacketHeader.SIZE (28 байт)             │   │
...
│  │   3. Создание PacketHeader (28 байт)                │   │
